...
* A submission on Canvas with a link to your GitHub project page **by the appropriate assignment deadline**. See the course assignment page for details on deadlines and lateness policy.
* At least 4 Git commits per chapter, with meaningful comments describing what was done. The content of the commits must indicate incremental progress through actual code for each chapter. There may be more than 4 commits, but there must be at least 4 commits with non-trivial changes to actual code.
...
Your application should function as expected without **errors** in the browser or on the command line, including linting errors. **Warnings** in the console or on the command line that do not affect the applicatiion's behavior may be ignored for the purpose of submitting the assignment (but I recommend at least reading all warnings).
...
* ~~The easiest way to connect your heroku app to a MongoDB database at present is to set up an mLab database through the Heroku add-ons interface. This involves a small change in your code: the `process.env.DB_URL` value in `db.js` must be changed to `process.env.MONGODB_URI`, which is the name of the environment variable that Heroku sets automatically when the mLab add-on is activated.~~ This option has been deprecated though, and ~~may not work in the future~~ doesn't work anymore. You can set up a database with Atlas and connect it by setting the database URL environment variable manually, as we discussed during lecture.
...
* I recommend creating a new OAuth 2.0 Client ID for your deployment application. Be sure to add the UI app's URI to the list of authorized URIs.
...
* In listing 14-1 on Page 472, the sign-in nav item class name is listed as `SigninNavItem`. Elsewhere (e.g. listing 14-2) it shows up as `SignInNavItem` with the `I` capitalized.
...
* In the changes between listing 12-33 and listing 12-39 the name of the variable `initialData` appears to have changed to `resultData`.
...
* On page 329 (Kindle location 8614 of 13609), with the addition of the Grid components, the code in listing 11-8 omits ellipses (`...`) in the `Page()` function that are usually used in the text to indicate code you don't touch. You need to keep the `NavBar` and `Footer` components as they were, so the function should look like this:
...
* On page 373 (Kindle location 9538 of 13609), the changes to `IssueList.jsx` should include deleting the line

        this.createIssue = this.createIssue.bind(this);

    However, it appears that the book omits a strike-through line in the code displayed here.

* Keep the code changes from Chapter 10 in mind when addiing `this.showError` to `handleSubmit` and `loadData` in listings 11-23 and 11-24. The calls to `parseInt` are still necessary.

* Listing 11-10 indicate that "NavLink" and "withRouter" imports from 'react-router-dom' should be removed. In fact "NavLink" and "Link" are the imports that should be removed, and "withRouter" should be left in place.
...
* Similarly to chapter 9, in listing 10-7, in the `loadData()` function in `IssueEdit.jsx`, a small change must be made in order to make the code work properly with recent versions of GraphQL. You need to change the line
    ...
    If you don't do this, your browser will display a server error complaining that an integer is expected but a numerical string is received. A similar change must be made in `handleSubmit`, where
    ...
    Keep these edits in mind, as this code will also come up in chapter 11.
...
* In listing 9-10 (pg 242) The caption mentions `IssueFilter.js`. This should read `IssueFilter.jsx`.
...
* In listing 9-21, in the `loadData()` function in `IssueDetail.jsx`, a small change must be made in order to make the code work properly with recent versions of GraphQL. You need to change the line
    ...
    If you don't do this, your browser will display a server error complaining that an integer is expected but a numerical string is received, along the lines of:

        INTERNAL_SERVER_ERROR: Variable "$id" got invalid value "1"; Expected type Int. Int cannot represent non-integer value: "1"
...
* In Listing 7-17, `rules` appears with no quotes. It should be `"rules"`.
...
In your readme update for chapter 6 include a screen shot of a local terminal running `mongo` on your own machine. Set it to use the issuetracker database and list the collections. The result should look like this (*note: your screenshot will not include `deleted_issues`. That will not be added to the database until chapter 10*):
...
![ch05](/readme_images/Ch05-graphql.png)
...
* Listing 2-1 has the div id as "contents". It should be "content" to go with the rest of the code described. (Either is fine, just be consistent).
...
* Listing 2-7 wrongly deletes the closing angle bracket after `</div` and also deletes the opening angle bracket before `script src`. The only edit you should be making is changing the script name and type. Angle brackets should remain as they were.
